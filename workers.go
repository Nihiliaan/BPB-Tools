...
func (sp ScriptUpdateParams) MarshalMultipart() ([]byte, string, error) {
	...
	if err != nil {
		return nil, "", fmt.Errorf("创建 metadata 部分失败：%w", err)
	}

	_, err = metadataPart.Write(metadataJSON)
	if err != nil {
		return nil, "", fmt.Errorf("写入 metadata 内容失败：%w", err)
	}
	...
	if err != nil {
		return nil, "", fmt.Errorf("创建 package-lock.json 部分失败：%w", err)
	}

	_, err = packageLockPart.Write([]byte("content of package-lock.json"))
	if err != nil {
		return nil, "", fmt.Errorf("写入 package-lock.json 内容失败：%w", err)
	}
	...
	if err != nil {
		return nil, "", fmt.Errorf("创建 package.json 部分失败：%w", err)
	}

	_, err = packageJSONPart.Write([]byte("content of package.json"))
	if err != nil {
		return nil, "", fmt.Errorf("写入 package.json 内容失败：%w", err)
	}
...
func createWorker(ctx context.Context, name string, uid string, pass string, proxy string, nat64Prefix string, fallback string, sub string, kv *kv.Namespace) (*workers.ScriptUpdateResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("序列化 multipart 数据失败：%w", err)
	}
...
func addWorkerCustomDomain(ctx context.Context, script string, customDomain string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("列出区域失败：%w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("更新 Worker 域名失败：%w", err)
	}
...
func listWorkers(ctx context.Context) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("列出 Workers 失败：%w", err)
	}
...
func deleteWorker(ctx context.Context, name string) error {
	...
	if err != nil {
		return fmt.Errorf("删除 Worker 失败：%w", err)
	}
...
func updateWorker(ctx context.Context, name string) error {
	...
	r := bytes.NewBuffer(data)
	if err != nil {
		return fmt.Errorf("序列化 multipart 数据失败：%w", err)
	}
	...
	if er != nil {
		return fmt.Errorf("更新 Worker 脚本失败：%w", er)
	}
...
func deployWorker(
	...
	if customDomain != "" {
		for {
			...
			successMessage("自定义域名已添加到 Worker！")
	...
	if err != nil {
		return "", fmt.Errorf("获取 Worker 子域名失败 - %w", err)
	}
